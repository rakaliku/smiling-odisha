  ...
  return (
    <footer className="relative mt-24 border-t border-border/60 bg-ink text-background">
      ...
      <div className="container relative py-14 grid gap-10 md:grid-cols-4">
        <div className="md:col-span-2">
          <div className="flex items-center gap-2 mb-4">
            ...
            <div>
              ...
              <div className="text-[10px] uppercase tracking-[0.2em] text-background/60">{t("Guidance · Service")}</div>
            </div>
          </div>
          <p className="text-sm text-background/70 max-w-md leading-relaxed">
            {t("Empowering Odisha's youth and communities through trusted career counseling and grassroots social work — rooted in our culture, building tomorrow.")}
          </p>
        ...
        <div>
          <h4 className="font-display text-base font-semibold mb-4 text-secondary">{t("Quick Links")}</h4>
          <ul className="space-y-2 text-sm text-background/70">
            <li><Link to="/" onClick={scrollToTop} className="hover:text-secondary transition-colors">{t("Home")}</Link></li>
            <li><Link to="/career" onClick={scrollToTop} className="hover:text-secondary transition-colors">{t("Career Counseling")}</Link></li>
            <li><Link to="/social-work" onClick={scrollToTop} className="hover:text-secondary transition-colors">{t("Social Work")}</Link></li>
            <li><Link to="/commerce-education" onClick={scrollToTop} className="hover:text-secondary transition-colors">{t("Commerce Class")}</Link></li>
            <li><Link to="/medha-sanman" onClick={scrollToTop} className="hover:text-secondary transition-colors">{t("Medha Sanman")}</Link></li>
          </ul>
        </div>

        <div>
          <h4 className="font-display text-base font-semibold mb-4 text-secondary">{t("Contact")}</h4>
          <ul className="space-y-3 text-sm text-background/70">
            <li className="flex items-start gap-2"><MapPin className="h-4 w-4 mt-0.5 text-primary-glow" /> {t("Plot No. HIG-3/5, Extension, Near Bhubaneswar Drug House, C.S. Pur, Bhubaneswar - 751016")}</li>
      ...
      <div className="relative border-t border-background/10 py-5 text-center text-xs text-background/50">
        © {new Date().getFullYear()} Smilling Odisha · {t("Made with ❤ for the people of Odisha")}
      </div>
    </footer>
  );
};
