...
import { medhaGalleryPhotos } from "@/data/medhaGalleryPhotos";
...
const MedhaSanman = () => {
  const { t } = useLanguage();
  useDocumentTitle(t("Medha Sanman — Honouring Excellence"));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(false);
    if (result.success) {
      toast.success(t("Nomination submitted! We'll review and get back to you soon."));
  ...
  return (
    <Layout>
      ...
      <section className="relative pattachitra-pattern py-20 md:py-28">
        <div className="container relative text-center max-w-3xl">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-primary/10 border border-primary/20 mb-5">
            <span className="text-xs font-semibold uppercase tracking-wider text-primary">{t("Honouring Excellence")}</span>
          </div>
          <h1 className="font-display text-4xl md:text-6xl font-bold leading-tight">
            {t("Medha Sanman")}
          </h1>
          <p className="mt-5 text-lg text-muted-foreground leading-relaxed max-w-2xl mx-auto">
            {t("Medha Sanman is our initiative to recognize and honour outstanding students for their academic excellence and achievements.")}
          </p>
        </div>
      </section>
      ...
      <section className="container py-16 md:py-20">
        <SectionHeading
          eyebrow={t("Our Purpose")}
          title={t("Objectives")}
          subtitle={t("Driving academic excellence and inspiring the next generation.")}
        />
          ...
          {objectives.map((o) => (
            <Card key={o.title} className="group bg-gradient-card border-border/60 hover:border-primary/40 hover:-translate-y-1 hover:shadow-warm transition-all">
              <CardContent className="p-7">
                ...
                <h3 className="font-display text-xl font-semibold mb-2">{t(o.title)}</h3>
                <p className="text-sm text-muted-foreground leading-relaxed">{t(o.desc)}</p>
              </CardContent>
            </Card>
          ))}
        </div>
      ...
      <section className="bg-gradient-warm py-20 relative">
        ...
        <div className="container relative">
          <SectionHeading
            eyebrow={t("Who Qualifies")}
            title={t("Eligibility Criteria")}
            subtitle={t("Medha Sanman celebrates achievers across these categories.")}
          />
            ...
            {eligibility.map((e) => (
              <Card key={e.title} className="group bg-card border-border/60 hover:border-primary/40 hover:-translate-y-1 hover:shadow-warm transition-all text-center">
                <CardContent className="p-7">
                  ...
                  <h3 className="font-display text-lg font-semibold mb-1.5">{t(e.title)}</h3>
                  <p className="text-sm text-muted-foreground leading-relaxed">{t(e.desc)}</p>
                </CardContent>
              </Card>
            ))}
          </div>
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-hero p-10 md:p-14 text-center shadow-warm">
          ...
          <div className="relative">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/20 backdrop-blur-sm border border-white/30 mb-5">
              ...
              <span className="text-xs font-semibold uppercase tracking-wider text-white">{t("Recognition & Rewards")}</span>
            </div>
            <h2 className="font-display text-3xl md:text-5xl font-bold text-white leading-tight mb-4">
              {t("Certificates, Trophies & Medals")}
            </h2>
            <p className="text-white/80 text-lg max-w-xl mx-auto mb-8 leading-relaxed">
              {t("Students receiving Medha Sanman are honoured with certificates, trophies, medals, and heartfelt appreciation for their hard work and dedication.")}
            </p>
            <a href="#nominate">
              <Button
                size="lg"
      ...
      {/* Photo Gallery */}
      {medhaGalleryPhotos.length > 0 && (
        <PhotoGallery
          photos={medhaGalleryPhotos}
          display="carousel"
          eyebrow={t("Photo Gallery")}
          title={t("Medha Sanman Moments")}
          subtitle={t("Celebrating Odisha's outstanding students and their achievements.")}
        />
      )}
      ...
      <section id="nominate" className="container py-20">
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          <div>
            <SectionHeading
              eyebrow={t("Nominate")}
              title={t("Submit a Nomination")}
              subtitle={t("Know a deserving student? Help us honour their achievements.")}
              align="left"
            />
            ...
            <CardContent className="p-7">
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="mname">{t("Student Name *")}</Label>
                    <Input id="mname" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder={t("Full name")} className="mt-1.5" />
                  </div>
                  <div>
                    <Label htmlFor="mphone">{t("Phone / WhatsApp *")}</Label>
                ...
                <div>
                  <Label htmlFor="memail">{t("Email *")}</Label>
                ...
                <div>
                  <Label htmlFor="mcategory">{t("Category *")}</Label>
                  <Select value={form.category} onValueChange={(v) => setForm({ ...form, category: v })}>
                    <SelectTrigger id="mcategory" className="mt-1.5">
                      <SelectValue placeholder={t("Select a category")} />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="school-topper">{t("School Topper")}</SelectItem>
                      <SelectItem value="college-topper">{t("College Topper")}</SelectItem>
                      <SelectItem value="competitive-exam">{t("Competitive Exam Achiever")}</SelectItem>
                      <SelectItem value="outstanding">{t("Outstanding Performer")}</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div>
                  <Label htmlFor="machievement">{t("Achievement Details *")}</Label>
                  <Input id="machievement" value={form.achievement} onChange={(e) => setForm({ ...form, achievement: e.target.value })} placeholder={t("e.g., 1st rank in Board Exam 2025")} className="mt-1.5" />
                </div>
                <div>
                  <Label htmlFor="mmsg">{t("Additional Details")}</Label>
                  <Textarea id="mmsg" rows={3} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} placeholder={t("Tell us more about the student's achievements...")} className="mt-1.5" />
                </div>
                <Button type="submit" disabled={submitting} className="w-full rounded-full bg-gradient-hero hover:opacity-90 shadow-warm h-11">
                  ...
                  {submitting ? t("Submitting...") : t("Submit Nomination")}
                </Button>
              </form>
            </CardContent>
          </Card>
